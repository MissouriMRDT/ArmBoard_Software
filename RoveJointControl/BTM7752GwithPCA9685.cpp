...
const RoveI2C_Speed DEFAULT_SPEED = I2CSPEED_FAST;
const uint8_t RESET_ADDRESS = 0b00000000;
const uint8_t RESET_MSG =     0b000001100;
const uint8_t M1REGISTER_ADDRESS = 0b00000000;
const uint8_t M1REGISTER_MSG =     0b00100000; //this will turn off sleep mode, turn on auto increment
const uint16_t PWM_MAX = 4095;
const uint8_t MAX_MOTOR_CHANNELS = 7;
...
: OutputDevice(InputPowerPercent, inverted)
{
  ...
  if(!initialized)
  {
    ...
    delay(1);
    roveI2cSend(i2cHandle, chipAddress, M1REGISTER_ADDRESS, M1REGISTER_MSG);
    delay(1);
...
void BTM7752GwithPCA9685::move(const long movement)
{
  if(enabled && movement != 0)
  {
    ...
    if(mov > 0)
    {
      roveI2cSendBurst(i2cHandle, chipAddress, channelRegisterBaseOffset+4, offMsg, 4);
      channelRegisterBaseOffset += 0;
    }
    else if(mov < 0)
    {
      roveI2cSendBurst(i2cHandle, chipAddress, channelRegisterBaseOffset, offMsg, 4);
      channelRegisterBaseOffset += 4;
    }

    uint8_t msg[4] = {onRegNum, onRegNum, (uint8_t)offRegNum, ((uint8_t)(offRegNum>>8))};
    roveI2cSendBurst(i2cHandle, chipAddress, channelRegisterBaseOffset, msg, 4);
  }
  else
  {
    stop();
...
void BTM7752GwithPCA9685::stop()
{
      ...
      uint8_t msg[4] = {onRegNum, onRegNum, (uint8_t)offRegNum, ((uint8_t)(offRegNum>>8))};
      roveI2cSendBurst(i2cHandle, chipAddress, channelRegisterBaseOffset, msg, 4);
      roveI2cSendBurst(i2cHandle, chipAddress, channelRegisterBaseOffset+4, msg, 4); //send to forward and reverse channels
