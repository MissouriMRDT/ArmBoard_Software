...
void GravityInertiaSystemStatus::update()
{
	...
	else if(Model == AtlasArm)
	{
	  ...
	  float th1 = consts->JOINT1ANGLE->getFeedback(); //todo:get to rads
	  float th2 = consts->JOINT2ANGLE->getFeedback();
	  float th3 = consts->JOINT3ANGLE->getFeedback();
    float th4 = consts->JOINT4ANGLE->getFeedback();
    float th5 = consts->JOINT5ANGLE->getFeedback();
    float th6 = consts->JOINT6ANGLE->getFeedback();
    ...
    AbicepCG[0][1] = 0;
    AbicepCG[0][2] = 0;
    AbicepCG[0][3] = consts->bcgz-consts->a2;
    AbicepCG[1][0] = 0;
    AbicepCG[1][1] = 1;
    AbicepCG[1][2] = 0;
...
void GravityInertiaSystemStatus::DHTrans(float th, float d, float a, float alpha, float A1[4][4]){  //Calculate the Homogenous transform from the DH convention
   A1[0][0] = cos(th);
