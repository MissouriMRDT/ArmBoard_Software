...
void DynamixelSendWriteCommand(Dynamixel dyna, uint8_t dynamixelRegister, uint8_t* data, uint8_t dataLength) {
...
uint8_t DynamixelRotateJoint(Dynamixel dyna, uint16_t position) {
  ...
  commands[0] = position & 0x00FF;
  commands[1] = position >> 8;
  
  DynamixelSendWriteCommand(dyna, DYNAMIXEL_GOAL_POSITION_L, commands, msgLength);
